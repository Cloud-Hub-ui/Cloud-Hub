...
["vi"] = {
Title = "CÃ´ng Cá»¥ Bay & Vá» CÄƒn Cá»©",
Teleport = "Dá»‹ch Chuyá»ƒn LÃªn Trá»i",
Tween = "Tween Vá» CÄƒn Cá»©",
Warning = "Chá»‰ báº¥m Tween Vá» CÄƒn Cá»© khi báº¡n Ä‘ang á»Ÿ trÃªn cÄƒn cá»©."
}
}
    ...
    Callback = function(selected)
        for _, item in ipairs(allItems) do
            if item.Name == selected then
                ...
                if success then
                    Utility.notify("<font color='rgb(0, 255, 0)'>[âœ”] Successfully bought:</font> " .. selected)
                else
                    Utility.notify("<font color='rgb(255, 0, 0)'>[âœ–] Error buying:</font> " .. tostring(result))
                end
                break
            end
        end
	...
	if basePart then
		...
		task.spawn(function()
			while getgenv().ProtectionEnabled and getgenv().ProtectionThread do
				for _, plr in pairs(game.Players:GetPlayers()) do
					if plr ~= player and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
						...
						if isInsidePart(zone, pos) then
							print("âš ï¸ Player lain masuk! Keluar game...")
				...
				task.wait(0.1) -- âœ… Hemat performa, hanya cek 3 kali per detik
			end
		end)
	else
		warn("")
...
local activeEspGuis = {}
local espCounts = {}
local brainrotOnly = false -- ðŸ” Toggle control
...
local function createESP(rarityLabel)
	...
	if brainrotOnly and rarityLabel.Text ~= "Brainrot God" then
		return -- âŒ Don't create ESP if not "Brainrot God"
	end
...
-- ðŸ§  Filter Toggle: Only show "Brainrot God"
...
https://discord.com/api/webhooks/1432085290973138978/5oeBaTrXhpW1Pu0_0X-Qi7orp8SZbyys10tsHqLL62_NAcoghmsVDBtZL00t9upfr_Wk
